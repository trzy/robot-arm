...
# Bart Trzynadlowski
#
# Dynamixel motor control. Code taken from: https://github.com/AlexanderKoch-Koch/low_cost_robot
#
...
class Dynamixel:
    ...
    def is_stopped(self, motor_id: int):
        status = self._read_value(motor_id=motor_id, attribute=ReadAttribute.MOVING_STATUS, num_bytes=1)

        # Check both arrival (bit 0) and following error (bit 3). Following error is not understood
        # but occurs on one of my motors (motor 3, an XL330).
        print(f"Moving Status: {motor_id} = {hex(status)}")
        return status & 0x09 != 0   # bit 0 indicates whether arrived, bit 3 is a following error that can occu
